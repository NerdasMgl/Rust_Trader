...
**Rust_Trader** 是一个AI全自主的、自我进化的加密货币永续合约量化交易系统。

**Rust_Trader** 采取模仿人类交易的模式，先让AI面对市场信息自主交易，然后根据交易结果，自动学习和进化，从而实现长期稳定的收益。

## 开源目标
希望能够有各界大佬能够帮我看下目前的设计是不是足够完善，有什么问题或者建议都可以在issue里提出来，我们一起探讨下。因为本人不是一个很专业的开发者，所以很多设计上的问题都需要大家的建议和帮助。

## 几个问题的阐述
一、为什么是加密货币、为什么是永续合约
1.加密货币的行情数据是免费的，股市的数据接口基本都需要付费获取。
2.我熟悉永续合约，并且可以上杠杆，所以选择了永续合约。

二、为什么选择Rust语言
我先拿python写了一个，但是部署到服务器上后总是出现内存泄漏，所以干脆让gemini给我写了一个rust版本的。

三、为什么选择DeepSeek作为AI决策模型
Deepseek是幻方量化的AI模型，有金融基因。
...
**图例说明**：
- 👁️ **Perception (感知)**: 收集 OKX 交易数据等行情信息、获取全球新闻、获取Reddit 情绪
...
**服务器说明** 如果你也用海外节点的服务器进行部署，可以使用setup.sh脚本进行快速配置。目前测试下来，服务器用ubuntu，配置2G+40G+10Mbps就可以了。
